package chess.practicaConnection;
...
import chess.basecode.bgame.control.ConsoleCtrl;
import chess.basecode.bgame.control.ConsoleCtrlMVC;
...
public class MainConnection {
	...
	private static void parseArgs(String[] args) {
		...
		cmdLineOptions.addOption(constructObstacleOption()); // -o
		...
		cmdLineOptions.addOption(constructApplicationModeOption()); // -am or --application-mode
	...
	//Constructor del comando obstaculos
	private static Option constructObstacleOption() {
		return new Option("o", "obstacles", true, "The number of squares occupied by obstacles"
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case ADVANCED_TIC_TAC_TOE:
			gameFactory = new AdvancedTTTFactoryExtended();
			break;
		case CONNECT_N:
			if (dimRows != null && dimCols != null && dimRows == dimCols) {
				gameFactory = new ConnectNFactoryExtended(dimRows);
			} else {
				gameFactory = new ConnectNFactoryExtended();
			}
			break;
		case TIC_TAC_TOE:
			gameFactory = new TicTacToeFactoryExtended();
			break;
		case ATAXX:
			if (dimRows != null && dimCols != null && dimRows == dimCols && numObs != null) {
				gameFactory = new AtaxxFactoryExtended(dimRows, numObs);
			}
			else if(dimRows == null && dimCols == null && numObs != null) {
				gameFactory = new AtaxxFactoryExtended(5, numObs);
			}
			else {
				gameFactory = new AtaxxFactoryExtended();
			}	
			break;
		default:
	...
	private static void parseApplicationModeOption(CommandLine line) throws ParseException {
		...
		String modeVal = line.getOptionValue("am");
		
		if (modeVal != null) {
			try {
				if (modeVal.toUpperCase().equals("NORMAL")) {
					applicationMode = ApplicationMode.NORMAL;
				} else if (modeVal.toUpperCase().equals("SERVER")) {
					applicationMode = ApplicationMode.SERVER;
				} else if (modeVal.toUpperCase().equals("CLIENT")) {
					applicationMode = ApplicationMode.CLIENT;
				}
			}
			catch (Exception e) {
				throw new ParseException("Invalid obstacles information");
			}
		}	
	}
	...
	private static void parseObstacleOption(CommandLine line) throws ParseException {
		String optionObs = line.getOptionValue("o");
		
		if (optionObs != null) {
			try {
				numObs = Integer.parseInt(optionObs);
			}
			catch (NumberFormatException e) {
				throw new ParseException("Invalid obstacles information");
			}
	...
	public static boolean isClientMode(){
		return applicationMode.getId() == "client";
	}
	...
	private static Option constructServerHostOption() {
		Option opt = new Option("sh", "server-host", true, "The server host");
		opt.setArgName("host");
		return opt;
	}

	private static void parseServerHostOption(CommandLine line) throws ParseException {	
		String optionHostOpt = line.getOptionValue("sh");
		
		if (optionHostOpt != null) {
			try {
				serverHost = optionHostOpt;
			}
			catch (NumberFormatException |IndexOutOfBoundsException e) {
				throw new ParseException("Invalid serverHost information");
			}
		}
	}
	
	private static Option constructServerPortOption() {
		Option opt = new Option("sp", "server-port", true, "The server port");
		opt.setArgName("port");
		return opt;
	}
	...
	private static void parseServerPortOption(CommandLine line) throws ParseException {
		...
		if (portVal != null) {
			try {
				...
				throw new ParseException("Invalid value for the server port '" + portVal + "'");
			}
		}
	}
}
